...
def bytes_to_archive(data: bytes) -> MPQArchive:
    replay_io = BytesIO()
    replay_io.write(data)
    replay_io.seek(0)
    return MPQArchive(replay_io)
...
class Replay:
    def __init__(self, replay_data: bytes) -> None:
        archive = bytes_to_archive(replay_data)
        ...
        self.details = protocol.decode_replay_details(
            content.get(b"replay.details") or content.get(b"replay.details.backup")
        )
        self.header = header
        self.metadata = json.loads(content[b"replay.gamemetadata.json"].decode(REPLAY_TYPE_ENCODING))

    @property
    def players(self) -> Sequence[dict]:
        return self.metadata["Players"]

    @property
    def cache_handles(self) -> Sequence[bytes]:
        return self.details["m_cacheHandles"]

    @property
    def map_name(self) -> str:
        return self.details["m_mapFileName"].decode(REPLAY_TYPE_ENCODING)

    @property
    def game_loops(self) -> int:
        return self.header["m_elapsedGameLoops"]

    @property
    def base_build(self) -> str:
        return self.metadata["BaseBuild"]

    @property
    def data_version(self) -> str:
        return self.metadata["DataVersion"]
    ...
    def update_battle_net_cache(self, battle_net_base: str) -> None:
        ...
        for cache_handle in self.cache_handles:
            cache_handle[4:8].decode(REPLAY_TYPE_ENCODING).strip("\x00 ")
            cache_hash = binascii.b2a_hex(cache_handle[8:]).decode(REPLAY_TYPE_ENCODING)
            file_type = cache_handle[0:4].decode(REPLAY_TYPE_ENCODING)
